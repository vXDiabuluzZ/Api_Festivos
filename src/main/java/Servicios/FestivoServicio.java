...
public class FestivoServicio implements IFestivoServicios {
    ...
    @Override
    public List<Entidades.datos.Festivodatos> obtenerFestivos(int año) {

        List<Festivo> festivos = repositorio.findAll();
        festivos = calcularFestivos(festivos, año);
        List<Festivodatos> fechas = new ArrayList<Festivodatos>();

        for (final Festivo festivo : festivos) {
            fechas.add(new Festivodatos(festivo.getNombre(), festivo.getFecha()));
        }
        return fechas;
    }
